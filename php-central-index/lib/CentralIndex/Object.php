<?php
    ...
    class CentralIndex_Object implements ArrayAccess{
        ...
        public function __construct($apiPublicKey=null, $apiPrivateKey=null){
            ...
            $this->_values = array();
            $this->_unsavedValues = new ChamberOfCommerce_Util_Set();
            $this->_transientValues = new ChamberOfCommerce_Util_Set();

        }

        public static function init(){
            self::$_permanentAttributes = new ChamberOfCommerce_Util_Set(array('_apiPublicKey','_apiPrivateKey'));
            self::$_nestedUpdatableAttributes = new ChamberOfCommerce_Util_Set(array('metadata'));
        }
        ...
        public function __get($k){
            if (array_key_exists($k, $this->_values)) {
                ...
                error_log("CentralIndex Notice: Undefined property of $class instance: $k.  HINT: The $k attribute was set in the past, however.  It was then wiped when refreshing the object with the result returned by CentralIndex's API, probably as a result of a save().  The attributes currently available on this object are: $attrs");
                return null;
            } else {
                $class = get_class($this);
                error_log("CentralIndex Notice: Undefined property of $class instance: $k");
                return null;
            }
        }
        ...
        public function refreshFrom($values, $apiPublicKey, $apiPrivateKey, $partial=false){
            ...
            if ($partial)
                $removed = new ChamberOfCommerce_Util_Set();
            else
            ...
            foreach ($values as $k => $v) {
                ...
                if (self::$_nestedUpdatableAttributes->includes($k) && is_array($v))
                    $this->_values[$k] = CentralIndex_Object::scopedConstructFrom('CentralIndex_AttachedObject', $v, $apiPublicKey, $apiPrivateKey);
                else
                    $this->_values[$k] = ChamberOfCommerce_Util::convertToChamberOfCommerceObject($v, $apiPublicKey, $apiPrivateKey);
        ...
        public function serializeParameters(){
            ...
            foreach (self::$_nestedUpdatableAttributes->toArray() as $property) {
                if (isset($this->$property) && $this->$property instanceOf CentralIndex_Object) {
        ...
        public function __toArray($recursive=false){
            if ($recursive)
                return ChamberOfCommerce_Util::convertChamberOfCommerceObjectToArray($this->_values);
            else
                return $this->_values;
        }
    }
